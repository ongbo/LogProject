...
//			System.out.println(value.toString());
			...
			// 获取事件名称
			clientInfo.put(EventLogConstants.LOG_COLUMN_NAME_EVENT_NAME,"e_l");
			...
			if(event==null) System.out.println("空·了");
			switch (event) {
			case LAUNCH:
			case PAGEVIEW:
			case CHARGEREQUEST:
	...
	private void handleData(Map<String, String> clientInfo, EventEnum event,
		...
		String memberId = clientInfo.get(EventLogConstants.LOG_COLUMN_NAME_MEMBER_ID);//会员名
...
//					put.add(family, Bytes.toBytes(entry.getKey()), Bytes.toBytes(entry.getValue()));
					...
					System.out.println(Bytes.toString(family)+"    "+entry.getKey().toString()+"      "+entry.getValue().toString());
				}
			}
